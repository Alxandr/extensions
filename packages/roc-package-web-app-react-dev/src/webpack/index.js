...
export default ({
    ...
    if (WEB) {
        newWebpackConfig.plugins.push(new webpack.IgnorePlugin(/^roc$/));
    }
    ...
    const hasApollo = !!(
        buildSettings.apollo && fileExists(buildSettings.apollo)
    );
    if (hasApollo) {
    ...
    const hasReducers = !!(
        buildSettings.reducers && fileExists(buildSettings.reducers)
    );
    if (hasReducers) {
    ...
    const hasMiddlewares = !!(
        buildSettings.redux.middlewares &&
        fileExists(buildSettings.redux.middlewares)
    );
    if (hasMiddlewares) {
    ...
    const hasEnhancers = !!(
        buildSettings.redux.enhancers &&
        fileExists(buildSettings.redux.enhancers)
    );
    if (hasEnhancers) {
    ...
    const hasSagas = !!(
        buildSettings.redux.sagas && fileExists(buildSettings.redux.sagas)
    );
    if (hasSagas) {
    ...
    const hasClientLoading = !!(
        buildSettings.clientLoading && fileExists(buildSettings.clientLoading)
    );
    if (hasClientLoading) {
    ...
    const hasTemplateValues = !!(
        buildSettings.templateValues && fileExists(buildSettings.templateValues)
    );
    ...
    const hasI18nLocales = !!(
        buildSettings.i18n.locales && buildSettings.i18n.locales.length
    );
    if (hasI18nLocales) {
